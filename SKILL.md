...
description: Generate signature images via the Signus API and return image files for chat delivery. Use when asked for signature generation through Signus API.
---
...
3. Request font generation from fixed trusted host (`https://api.signus.ai`) using only:
   - `POST /api/signus/v0/signature-generations/font`
4. Require ZIP/octet-stream response, then extract with in-process JS unzip library (`adm-zip`) and collect image files.
5. Return:
   - `count`
   - `directory`
...
- It uses only `POST /api/signus/v0/signature-generations/font` and expects archive output.
